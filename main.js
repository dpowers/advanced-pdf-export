...
${r}`),r=n.insertAutoBreaks(r,e.autoBreakH1,e.autoBreakH2);let i=ie(r),l=O[e.pageSize]??O.A4,o=e.orientation==="landscape"?l.h:l.w,a=e.orientation==="landscape"?l.w:l.h,d=A(e.marginTop),c=A(e.marginBottom),p=A(e.marginLeft),f=A(e.marginRight),u=e.showFooter?28:0,m=e.showHeader&&e.headerText?20:0,C=o-p-f,y=a-d-c-u-m,E=oe(this.editorEl.value),x=pe(e,E),k=this.currentFile?.path??"pdf-export",h=await Promise.all(i.map(B=>le(this.app,B.trim(),k,this.renderComponent)));if(t!==this.renderToken)return;let g=[];for(let B of h)g.push(...Se(B,C,y,x));let b=Le(g,e),w=e.fontFamily==="__custom__"?e.customFontName.trim()||"inherit":e.fontFamily;this.layoutCache={layouts:b,pw:o,ph:a,mTop:d,mLeft:p,footerH:u,headerH:m,contentW:C,contentH:y,docCSS:x,fontFamily:w,accentColor:e.accentColor,pageBackground:e.pageBackground,isRTL:E},this.drawPreview(this.layoutCache,e.previewScale),this.pageCountEl.textContent=`${b.length} page${b.length!==1?"s":""}`,this.hideLoading()}renderPreviewOnly(){this.layoutCache&&this.drawPreview(this.layoutCache,this.plugin.settings.previewScale)}showLoading(){this.loadingOverlayEl.addClass("is-active"),this.renderBtn.disabled=!0,this.renderBtn.textContent="Rendering\u2026"}hideLoading(){this.loadingOverlayEl.removeClass("is-active"),this.renderBtn.disabled=!1,this.renderBtn.textContent="\u27F3 Render PDF"}drawPreview(t,e){let{layouts:r,pw:i,ph:l,mTop:o,mLeft:a,footerH:d,headerH:c,contentW:p,docCSS:f,fontFamily:u,accentColor:m,pageBackground:C,isRTL:y}=t,E=this.plugin.settings;this.previewEl.empty();let x=new Map;r.forEach((h,g)=>{for(let b of h.pageNodes)b.querySelectorAll("[id]").forEach(w=>{x.has(w.id)||x.set(w.id,g)}),b.id&&!x.has(b.id)&&x.set(b.id,g)});let k=[];for(let h of r){let g=Math.round(i*e),b=Math.round(l*e),w=this.previewEl.createEl("div",{cls:"mpdf-page-wrap"});w.style.cssText=`width:${g}px;height:${b}px;`,w.createEl("div",{cls:"mpdf-page-label",text:`Page ${h.pageNum} of ${h.totalPages}`}),k.push(w);let B=w.createEl("div",{cls:"mpdf-page-scale"});B.style.cssText=`width:${g}px;height:${b}px;`;let F=activeDocument.createElement("div");F.addClass("mpdf-shadow-host"),F.style.cssText=`width:${i}px;height:${l}px;transform:scale(${e});`,B.appendChild(F);let H=F.attachShadow({mode:"open"}),D=`
        :host {
          display: block;
          width: ${i}px;
          height: ${l}px;
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        ${f}
      `,N=new CSSStyleSheet;if(N.replaceSync(D),H.adoptedStyleSheets=[N],E.showHeader&&(h.headerLeft||h.headerCenter||h.headerRight)){let T=activeDocument.createElement("div");if(T.style.cssText=["position:absolute",`top:${o*.4}px`,`left:${a}px`,`right:${a}px`,"display:flex","align-items:center","font-size:9px","color:#999",`font-family:${u}`,"white-space:nowrap"].join(";"),h.headerCenter){let v=activeDocument.createElement("span");v.className="mpdf-hf-center",v.textContent=h.headerCenter,T.appendChild(v)}else{let v=activeDocument.createElement("span");v.textContent=h.headerLeft,T.appendChild(v);let L=activeDocument.createElement("span");L.className="mpdf-hf-right",L.textContent=h.headerRight,T.appendChild(L)}H.appendChild(T)}let S=activeDocument.createElement("div");S.className="mpdf-doc",y&&S.setAttribute("dir","rtl"),S.style.cssText=["position:absolute",`top:${o+c}px`,`left:${a}px`,`width:${p}px`].join(";");for(let T of h.pageNodes)S.appendChild(T.cloneNode(!0));if(H.appendChild(S),S.querySelectorAll("a[href^='#']").forEach(T=>{let v=decodeURIComponent(T.getAttribute("href").slice(1)),L=x.get(v);L!==void 0&&(T.title=`Go to page ${L+1}`,T.addEventListener("click",G=>{G.preventDefault(),k[L]?.scrollIntoView({behavior:"smooth",block:"start"})}))}),E.showFooter&&(h.footerLeft||h.footerRight||h.footerCenter)){let T=activeDocument.createElement("div");if(T.style.cssText=["position:absolute","bottom:0","left:0","right:0",`height:${d}px`,"display:flex","align-items:center",E.showFooterBorder?`border-top:0.5px solid ${m}33`:"",`padding:0 ${a}px`,"font-size:9px","color:#aaa",`font-family:${u}`].filter(Boolean).join(";"),h.footerCenter){let v=activeDocument.createElement("span");v.className="mpdf-hf-center",v.textContent=h.footerCenter,T.appendChild(v)}else{let v=activeDocument.createElement("span");v.textContent=h.footerLeft,T.appendChild(v);let L=activeDocument.createElement("span");L.className="mpdf-hf-right",L.textContent=h.footerRight,T.appendChild(L)}H.appendChild(T)}}}async exportPDF(){let t=this.plugin.settings;this.exportBtn.disabled=!0,this.exportBtn.textContent="\u2B07 Exporting\u2026";let e=()=>{this.exportBtn.disabled=!1,this.exportBtn.textContent="\u2B07 Export PDF"};if(!this.layoutCache){this.renderDebounceTimer!==null&&(window.clearTimeout(this.renderDebounceTimer),this.renderDebounceTimer=null);let g=++this.renderToken;this.showLoading();try{await this.doRender(g)}catch(b){let w=b instanceof Error?b.message:String(b);new s.Notice("Render error: "+w),this.hideLoading(),e();return}}let r=this.layoutCache;if(!r||r.layouts.length===0){new s.Notice("Nothing to export."),e();return}let{layouts:i,pw:l,ph:o,mTop:a,mLeft:d,footerH:c,headerH:p,contentW:f,docCSS:u,fontFamily:m,accentColor:C,pageBackground:y,isRTL:E}=r,x=i.map(g=>{let b=g.pageNodes.map(P=>P.outerHTML).join(`
`),w=t.showHeader&&(g.headerLeft||g.headerCenter||g.headerRight),B=g.headerCenter?`<span style="flex:1;text-align:center;">${$(g.headerCenter)}</span>`:`<span>${$(g.headerLeft)}</span><span style="margin-left:auto;">${$(g.headerRight)}</span>`,F=w?`<div style="position:absolute;top:${a*.4}px;left:${d}px;right:${d}px;display:flex;align-items:center;font-size:9px;color:#999;font-family:${m};white-space:nowrap;">${B}</div>`:"",H=g.footerCenter?`<span style="flex:1;text-align:center;">${$(g.footerCenter)}</span>`:`<span>${$(g.footerLeft)}</span><span style="margin-left:auto;">${$(g.footerRight)}</span>`,D=t.showFooter&&(g.footerLeft||g.footerRight||g.footerCenter),N=t.showFooterBorder?`border-top:0.5px solid ${C}33;`:"",M=D?`<div style="position:absolute;bottom:0;left:0;right:0;height:${c}px;display:flex;align-items:center;${N}padding:0 ${d}px;font-size:9px;color:#aaa;font-family:${m};">${H}</div>`:"",S=`<div class="mpdf-doc"${E?' dir="rtl"':""} style="position:absolute;top:${a+p}px;left:${d}px;width:${f}px;">${b}</div>`;return`<div class="mpdf-export-page">${F}${S}${M}</div>`}),k=`
