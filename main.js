...
${w}`:w,m=[];for(let x of N)m.push(...Pe(x,T,S,g));let C=Ae(m,e),E=e.fontFamily==="__custom__"?e.customFontName.trim()||"inherit":e.fontFamily;this.layoutCache={layouts:C,pw:l,ph:a,mTop:o,mLeft:p,mRight:h,footerH:u,headerH:b,contentW:T,contentH:S,docCSS:g,fontFamily:E,accentColor:e.accentColor,pageBackground:e.pageBackground,isRTL:k},this.drawPreview(this.layoutCache,e.previewScale),this.pageCountEl.textContent=`${C.length} page${C.length!==1?"s":""}`,this.hideLoading()}renderPreviewOnly(){this.layoutCache&&this.drawPreview(this.layoutCache,this.plugin.settings.previewScale)}showLoading(){this.loadingOverlayEl.addClass("mpdf-is-active"),this.renderBtn.disabled=!0,this.renderBtn.textContent="Rendering\u2026"}hideLoading(){this.loadingOverlayEl.removeClass("mpdf-is-active"),this.renderBtn.disabled=!1,this.renderBtn.textContent="\u27F3 Render PDF"}drawPreview(n,e){let{layouts:r,pw:i,ph:s,mTop:l,mLeft:a,mRight:o,footerH:c,headerH:p,contentW:h,docCSS:u,fontFamily:b,accentColor:T,pageBackground:S,isRTL:k}=n,w=this.plugin.settings;this.previewEl.empty();let F=new Map;r.forEach((m,C)=>{for(let E of m.pageNodes)E.querySelectorAll("[id]").forEach(x=>{F.has(x.id)||F.set(x.id,C)}),E.id&&!F.has(E.id)&&F.set(E.id,C)});let N=[],f=`
      :host {
        display: block;
        width: ${i}px;
        height: ${s}px;
