...
`)}async doRender(t){var k,g,E;let e=this.plugin.settings,o=ne(this.editorEl.value);e.hideFrontmatter&&(o=oe(o)),e.includeFilenameAsTitle&&this.currentFile&&(o=`# ${this.currentFile.basename}

${o}`),o=A.insertAutoBreaks(o,e.autoBreakH1,e.autoBreakH2);let s=re(o),l=(k=z[e.pageSize])!=null?k:z.A4,r=e.orientation==="landscape"?l.h:l.w,a=e.orientation==="landscape"?l.w:l.h,c=R(e.marginTop),p=R(e.marginBottom),h=R(e.marginLeft),u=R(e.marginRight),b=e.showFooter?28:0,m=e.showHeader&&e.headerText?20:0,T=r-h-u,y=a-c-p-b-m,C=de(e),B=(E=(g=this.currentFile)==null?void 0:g.path)!=null?E:"pdf-export",M=await Promise.all(s.map(L=>ae(this.app,L.trim(),B,this.renderComponent)));if(t!==this.renderToken)return;let f=[];for(let L of M)f.push(...He(L,T,y,C));let v=ve(f,e),S=e.fontFamily==="__custom__"?e.customFontName.trim()||"inherit":e.fontFamily;this.layoutCache={layouts:v,pw:r,ph:a,mTop:c,mLeft:h,footerH:b,headerH:m,contentW:T,contentH:y,docCSS:C,fontFamily:S,accentColor:e.accentColor,pageBackground:e.pageBackground},this.drawPreview(this.layoutCache,e.previewScale),this.pageCountEl.textContent=`${v.length} page${v.length!==1?"s":""}`,this.hideLoading()}renderPreviewOnly(){this.layoutCache&&this.drawPreview(this.layoutCache,this.plugin.settings.previewScale)}showLoading(){this.loadingOverlayEl.style.display="flex",this.renderBtn.disabled=!0,this.renderBtn.textContent="Rendering\u2026"}hideLoading(){this.loadingOverlayEl.style.display="none",this.renderBtn.disabled=!1,this.renderBtn.textContent="\u27F3 Render PDF"}drawPreview(t,e){let{layouts:o,pw:s,ph:l,mTop:r,mLeft:a,footerH:c,headerH:p,contentW:h,contentH:u,docCSS:b,fontFamily:m,accentColor:T,pageBackground:y}=t,C=this.plugin.settings;this.previewEl.empty();let B=new Map;o.forEach((f,v)=>{for(let S of f.pageNodes)S.querySelectorAll("[id]").forEach(k=>{B.has(k.id)||B.set(k.id,v)}),S.id&&!B.has(S.id)&&B.set(S.id,v)});let M=[];for(let f of o){let v=Math.round(s*e),S=Math.round(l*e),k=this.previewEl.createEl("div",{cls:"mpdf-page-wrap"});k.style.cssText=`width:${v}px;height:${S}px;position:relative;flex-shrink:0;`,k.createEl("div",{cls:"mpdf-page-label",text:`Page ${f.pageNum} of ${f.totalPages}`}),M.push(k);let g=k.createEl("div",{cls:"mpdf-page-scale"});g.style.cssText=`width:${v}px;height:${S}px;overflow:hidden;position:relative;`;let E=document.createElement("div");E.style.cssText=[`width:${s}px`,`height:${l}px`,`transform:scale(${e})`,"transform-origin:top left","position:absolute","top:0","left:0"].join(";"),g.appendChild(E);let L=E.attachShadow({mode:"open"}),N=document.createElement("style");if(N.textContent=`
        :host {
          display: block;
          width: ${s}px;
          height: ${l}px;
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ${b}
      `,L.appendChild(N),C.showHeader&&(f.headerLeft||f.headerCenter||f.headerRight)){let w=document.createElement("div");if(w.style.cssText=["position:absolute",`top:${r*.4}px`,`left:${a}px`,`right:${a}px`,"display:flex","align-items:center","font-size:9px","color:#999",`font-family:${m}`,"white-space:nowrap"].join(";"),f.headerCenter){let x=document.createElement("span");x.style.cssText="flex:1;text-align:center;",x.textContent=f.headerCenter,w.appendChild(x)}else{let x=document.createElement("span");x.textContent=f.headerLeft,w.appendChild(x);let H=document.createElement("span");H.style.marginLeft="auto",H.textContent=f.headerRight,w.appendChild(H)}L.appendChild(w)}let F=document.createElement("div");F.className="mpdf-doc",F.style.cssText=["position:absolute",`top:${r+p}px`,`left:${a}px`,`width:${h}px`,`height:${u}px`,"overflow:hidden"].join(";");for(let w of f.pageNodes)F.appendChild(w.cloneNode(!0));if(L.appendChild(F),F.querySelectorAll("a[href^='#']").forEach(w=>{let x=decodeURIComponent(w.getAttribute("href").slice(1)),H=B.get(x);H!==void 0&&(w.title=`Go to page ${H+1}`,w.addEventListener("click",V=>{var K;V.preventDefault(),(K=M[H])==null||K.scrollIntoView({behavior:"smooth",block:"start"})}))}),C.showFooter&&(f.footerLeft||f.footerRight||f.footerCenter)){let w=document.createElement("div");if(w.style.cssText=["position:absolute","bottom:0","left:0","right:0",`height:${c}px`,"display:flex","align-items:center",C.showFooterBorder?`border-top:0.5px solid ${T}33`:"",`padding:0 ${a}px`,"font-size:9px","color:#aaa",`font-family:${m}`].filter(Boolean).join(";"),f.footerCenter){let x=document.createElement("span");x.style.cssText="flex:1;text-align:center;",x.textContent=f.footerCenter,w.appendChild(x)}else{let x=document.createElement("span");x.textContent=f.footerLeft,w.appendChild(x);let H=document.createElement("span");H.style.marginLeft="auto",H.textContent=f.footerRight,w.appendChild(H)}L.appendChild(w)}}}async exportPDF(){var f,v,S,k;let t=this.plugin.settings;this.exportBtn.disabled=!0,this.exportBtn.textContent="\u2B07 Exporting\u2026";let e=()=>{this.exportBtn.disabled=!1,this.exportBtn.textContent="\u2B07 Export PDF"};if(!this.layoutCache){this.renderDebounceTimer!==null&&(clearTimeout(this.renderDebounceTimer),this.renderDebounceTimer=null);let g=++this.renderToken;this.showLoading();try{await this.doRender(g)}catch(E){let L=E instanceof Error?E.message:String(E);new d.Notice("Render error: "+L),this.hideLoading(),e();return}}let o=this.layoutCache;if(!o||o.layouts.length===0){new d.Notice("Nothing to export."),e();return}let{layouts:s,pw:l,ph:r,mTop:a,mLeft:c,footerH:p,headerH:h,contentW:u,docCSS:b,fontFamily:m,accentColor:T,pageBackground:y}=o,C=s.map(g=>{let E=g.pageNodes.map(V=>V.outerHTML).join(`
`),L=t.showHeader&&(g.headerLeft||g.headerCenter||g.headerRight),N=g.headerCenter?`<span style="flex:1;text-align:center;">${D(g.headerCenter)}</span>`:`<span>${D(g.headerLeft)}</span><span style="margin-left:auto;">${D(g.headerRight)}</span>`,$=L?`<div style="position:absolute;top:${a*.4}px;left:${c}px;right:${c}px;display:flex;align-items:center;font-size:9px;color:#999;font-family:${m};white-space:nowrap;">${N}</div>`:"",F=g.footerCenter?`<span style="flex:1;text-align:center;">${D(g.footerCenter)}</span>`:`<span>${D(g.footerLeft)}</span><span style="margin-left:auto;">${D(g.footerRight)}</span>`,P=t.showFooter&&(g.footerLeft||g.footerRight||g.footerCenter),w=t.showFooterBorder?`border-top:0.5px solid ${T}33;`:"",x=P?`<div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;display:flex;align-items:center;${w}padding:0 ${c}px;font-size:9px;color:#aaa;font-family:${m};">${F}</div>`:"",H=`<div class="mpdf-doc" style="position:absolute;top:${a+h}px;left:${c}px;width:${u}px;">${E}</div>`;return`<div class="mpdf-export-page">${$}${H}${x}</div>`}),B=`
...
<head>
<meta charset="UTF-8">
<title>${D((v=(f=this.currentFile)==null?void 0:f.basename)!=null?v:"Export")}</title>
<style>${B}</style>
</head>
<body>
${C.join(`
