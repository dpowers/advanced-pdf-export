...
`)}async doRender(t){var M,g,E;let e=this.plugin.settings,o=te(this.editorEl.value);e.hideFrontmatter&&(o=ne(o)),e.includeFilenameAsTitle&&this.currentFile&&(o=`# ${this.currentFile.basename}

${o}`),o=A.insertAutoBreaks(o,e.autoBreakH1,e.autoBreakH2);let s=oe(o),l=(M=I[e.pageSize])!=null?M:I.A4,r=e.orientation==="landscape"?l.h:l.w,a=e.orientation==="landscape"?l.w:l.h,c=O(e.marginTop),p=O(e.marginBottom),h=O(e.marginLeft),f=O(e.marginRight),b=e.showFooter?28:0,m=e.showHeader&&e.headerText?20:0,C=r-h-f,y=a-c-p-b-m,T=se(e),B=(E=(g=this.currentFile)==null?void 0:g.path)!=null?E:"pdf-export",u=await Promise.all(s.map(F=>re(this.app,F.trim(),B,this.renderComponent)));if(t!==this.renderToken)return;let S=[];for(let F of u)S.push(...Ee(F,C,y,T));let v=He(S,e),L=e.fontFamily==="__custom__"?e.customFontName.trim()||"inherit":e.fontFamily;this.layoutCache={layouts:v,pw:r,ph:a,mTop:c,mLeft:h,footerH:b,headerH:m,contentW:C,contentH:y,docCSS:T,fontFamily:L,accentColor:e.accentColor,pageBackground:e.pageBackground},this.drawPreview(this.layoutCache,e.previewScale),this.pageCountEl.textContent=`${v.length} page${v.length!==1?"s":""}`,this.hideLoading()}renderPreviewOnly(){this.layoutCache&&this.drawPreview(this.layoutCache,this.plugin.settings.previewScale)}showLoading(){this.loadingOverlayEl.style.display="flex",this.renderBtn.disabled=!0,this.renderBtn.textContent="Rendering\u2026"}hideLoading(){this.loadingOverlayEl.style.display="none",this.renderBtn.disabled=!1,this.renderBtn.textContent="\u27F3 Render PDF"}drawPreview(t,e){let{layouts:o,pw:s,ph:l,mTop:r,mLeft:a,footerH:c,headerH:p,contentW:h,docCSS:f,fontFamily:b,accentColor:m,pageBackground:C}=t,y=this.plugin.settings;this.previewEl.empty();let T=new Map;o.forEach((u,S)=>{for(let v of u.pageNodes)v.querySelectorAll("[id]").forEach(L=>{T.has(L.id)||T.set(L.id,S)}),v.id&&!T.has(v.id)&&T.set(v.id,S)});let B=[];for(let u of o){let S=Math.round(s*e),v=Math.round(l*e),L=this.previewEl.createEl("div",{cls:"mpdf-page-wrap"});L.style.cssText=`width:${S}px;height:${v}px;position:relative;flex-shrink:0;`,L.createEl("div",{cls:"mpdf-page-label",text:`Page ${u.pageNum} of ${u.totalPages}`}),B.push(L);let M=L.createEl("div",{cls:"mpdf-page-scale"});M.style.cssText=`width:${S}px;height:${v}px;overflow:hidden;position:relative;`;let g=document.createElement("div");g.style.cssText=[`width:${s}px`,`height:${l}px`,`transform:scale(${e})`,"transform-origin:top left","position:absolute","top:0","left:0"].join(";"),M.appendChild(g);let E=g.attachShadow({mode:"open"}),F=document.createElement("style");if(F.textContent=`
        :host {
          display: block;
          width: ${s}px;
          height: ${l}px;
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ${f}
      `,E.appendChild(F),y.showHeader&&(u.headerLeft||u.headerCenter||u.headerRight)){let w=document.createElement("div");if(w.style.cssText=["position:absolute",`top:${r*.4}px`,`left:${a}px`,`right:${a}px`,"display:flex","align-items:center","font-size:9px","color:#999",`font-family:${b}`,"white-space:nowrap"].join(";"),u.headerCenter){let x=document.createElement("span");x.style.cssText="flex:1;text-align:center;",x.textContent=u.headerCenter,w.appendChild(x)}else{let x=document.createElement("span");x.textContent=u.headerLeft,w.appendChild(x);let H=document.createElement("span");H.style.marginLeft="auto",H.textContent=u.headerRight,w.appendChild(H)}E.appendChild(w)}let k=document.createElement("div");k.className="mpdf-doc",k.style.cssText=["position:absolute",`top:${r+p}px`,`left:${a}px`,`width:${h}px`].join(";");for(let w of u.pageNodes)k.appendChild(w.cloneNode(!0));if(E.appendChild(k),k.querySelectorAll("a[href^='#']").forEach(w=>{let x=decodeURIComponent(w.getAttribute("href").slice(1)),H=T.get(x);H!==void 0&&(w.title=`Go to page ${H+1}`,w.addEventListener("click",P=>{var R;P.preventDefault(),(R=B[H])==null||R.scrollIntoView({behavior:"smooth",block:"start"})}))}),y.showFooter&&(u.footerLeft||u.footerRight||u.footerCenter)){let w=document.createElement("div");if(w.style.cssText=["position:absolute","bottom:0","left:0","right:0",`height:${c}px`,"display:flex","align-items:center",y.showFooterBorder?`border-top:0.5px solid ${m}33`:"",`padding:0 ${a}px`,"font-size:9px","color:#aaa",`font-family:${b}`].filter(Boolean).join(";"),u.footerCenter){let x=document.createElement("span");x.style.cssText="flex:1;text-align:center;",x.textContent=u.footerCenter,w.appendChild(x)}else{let x=document.createElement("span");x.textContent=u.footerLeft,w.appendChild(x);let H=document.createElement("span");H.style.marginLeft="auto",H.textContent=u.footerRight,w.appendChild(H)}E.appendChild(w)}}}async exportPDF(){var S,v,L,M;let t=this.plugin.settings;this.exportBtn.disabled=!0,this.exportBtn.textContent="\u2B07 Exporting\u2026";let e=()=>{this.exportBtn.disabled=!1,this.exportBtn.textContent="\u2B07 Export PDF"};if(!this.layoutCache){this.renderDebounceTimer!==null&&(clearTimeout(this.renderDebounceTimer),this.renderDebounceTimer=null);let g=++this.renderToken;this.showLoading();try{await this.doRender(g)}catch(E){let F=E instanceof Error?E.message:String(E);new d.Notice("Render error: "+F),this.hideLoading(),e();return}}let o=this.layoutCache;if(!o||o.layouts.length===0){new d.Notice("Nothing to export."),e();return}let{layouts:s,pw:l,ph:r,mTop:a,mLeft:c,footerH:p,headerH:h,contentW:f,docCSS:b,fontFamily:m,accentColor:C,pageBackground:y}=o,T=s.map(g=>{let E=g.pageNodes.map(R=>R.outerHTML).join(`
`),F=t.showHeader&&(g.headerLeft||g.headerCenter||g.headerRight),D=g.headerCenter?`<span style="flex:1;text-align:center;">${N(g.headerCenter)}</span>`:`<span>${N(g.headerLeft)}</span><span style="margin-left:auto;">${N(g.headerRight)}</span>`,k=F?`<div style="position:absolute;top:${a*.4}px;left:${c}px;right:${c}px;display:flex;align-items:center;font-size:9px;color:#999;font-family:${m};white-space:nowrap;">${D}</div>`:"",$=g.footerCenter?`<span style="flex:1;text-align:center;">${N(g.footerCenter)}</span>`:`<span>${N(g.footerLeft)}</span><span style="margin-left:auto;">${N(g.footerRight)}</span>`,w=t.showFooter&&(g.footerLeft||g.footerRight||g.footerCenter),x=t.showFooterBorder?`border-top:0.5px solid ${C}33;`:"",H=w?`<div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;display:flex;align-items:center;${x}padding:0 ${c}px;font-size:9px;color:#aaa;font-family:${m};">${$}</div>`:"",P=`<div class="mpdf-doc" style="position:absolute;top:${a+h}px;left:${c}px;width:${f}px;">${E}</div>`;return`<div class="mpdf-export-page">${k}${P}${H}</div>`}),B=`
...
<head>
<meta charset="UTF-8">
<title>${N((v=(S=this.currentFile)==null?void 0:S.basename)!=null?v:"Export")}</title>
<style>${B}</style>
</head>
<body>
${T.join(`
