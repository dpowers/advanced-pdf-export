...
${a}`),a=t.insertAutoBreaks(a,e.autoBreakH1,e.autoBreakH2);let i=le(a),s=q[e.pageSize]??q.A4,o=e.orientation==="landscape"?s.h:s.w,r=e.orientation==="landscape"?s.w:s.h,d=z(e.marginTop),c=z(e.marginBottom),p=z(e.marginLeft),h=z(e.marginRight),f=e.showFooter?28:0,m=e.showHeader&&e.headerText?20:0,w=Math.max(1,o-p-h),T=Math.max(1,r-d-c-f-m),S=ae(this.editorEl.value),v=ue(e,S),k=this.currentFile?.path??"pdf-export",M=await Promise.all(i.map(C=>de(this.app,C,k,this.renderComponent)));if(n!==this.renderToken)return;let N=we(),u=N?`${N}
${v}`:v,g=[];for(let C of M)g.push(...$e(C,w,T,u));let y=Pe(g,e),E=e.fontFamily==="__custom__"?e.customFontName.trim()||"inherit":e.fontFamily;this.layoutCache={layouts:y,pw:o,ph:r,mTop:d,mLeft:p,mRight:h,footerH:f,headerH:m,contentW:w,contentH:T,docCSS:u,fontFamily:E,accentColor:e.accentColor,pageBackground:e.pageBackground,isRTL:S},this.drawPreview(this.layoutCache,e.previewScale),this.pageCountEl.textContent=`${y.length} page${y.length!==1?"s":""}`,this.hideLoading()}renderPreviewOnly(){this.layoutCache&&this.drawPreview(this.layoutCache,this.plugin.settings.previewScale)}showLoading(){this.loadingOverlayEl.addClass("is-active"),this.renderBtn.disabled=!0,this.renderBtn.textContent="Rendering\u2026"}hideLoading(){this.loadingOverlayEl.removeClass("is-active"),this.renderBtn.disabled=!1,this.renderBtn.textContent="\u27F3 Render PDF"}drawPreview(n,e){let{layouts:a,pw:i,ph:s,mTop:o,mLeft:r,mRight:d,footerH:c,headerH:p,contentW:h,docCSS:f,fontFamily:m,accentColor:w,pageBackground:T,isRTL:S}=n,v=this.plugin.settings;this.previewEl.empty();let k=new Map;a.forEach((g,y)=>{for(let E of g.pageNodes)E.querySelectorAll("[id]").forEach(C=>{k.has(C.id)||k.set(C.id,y)}),E.id&&!k.has(E.id)&&k.set(E.id,y)});let M=[],N=`
      :host {
        display: block;
        width: ${i}px;
        height: ${s}px;
      ...
      *, *::before, *::after { box-sizing: border-box; }
      ...
      ${f}
    `,u=new CSSStyleSheet;u.replaceSync(N);for(let g of a){let y=Math.round(i*e),E=Math.round(s*e),C=this.previewEl.createEl("div",{cls:"mpdf-page-wrap"});C.setCssStyles({width:`${y}px`,height:`${E}px`}),C.createEl("div",{cls:"mpdf-page-label",text:`Page ${g.pageNum} of ${g.totalPages}`}),M.push(C);let F=C.createEl("div",{cls:"mpdf-page-scale"});F.setCssStyles({width:`${y}px`,height:`${E}px`});let B=activeDocument.createElement("div");B.addClass("mpdf-shadow-host"),B.setCssStyles({width:`${i}px`,height:`${s}px`,transform:`scale(${e})`}),F.appendChild(B);let D=B.attachShadow({mode:"open"});if(D.adoptedStyleSheets=[u],v.showHeader&&(g.headerLeft||g.headerCenter||g.headerRight)){let b=activeDocument.createElement("div");if(b.setCssStyles({position:"absolute",top:`${o*.4}px`,left:`${r}px`,right:`${d}px`,display:"flex",alignItems:"center",fontSize:"9px",color:"#999",fontFamily:m,whiteSpace:"nowrap"}),g.headerCenter){let x=activeDocument.createElement("span");x.className="mpdf-hf-center",x.textContent=g.headerCenter,b.appendChild(x)}else{let x=activeDocument.createElement("span");x.textContent=g.headerLeft,b.appendChild(x);let L=activeDocument.createElement("span");L.className="mpdf-hf-right",L.textContent=g.headerRight,b.appendChild(L)}D.appendChild(b)}let H=activeDocument.createElement("div");H.className="mpdf-doc",S&&H.setAttribute("dir","rtl"),H.setCssStyles({position:"absolute",top:`${o+p}px`,left:`${r}px`,width:`${h}px`});for(let b of g.pageNodes)H.appendChild(b.cloneNode(!0));if(D.appendChild(H),H.querySelectorAll("a[href^='#']").forEach(b=>{let x=decodeURIComponent(b.getAttribute("href").slice(1)),L=k.get(x);L!==void 0&&(b.title=`Go to page ${L+1}`,b.addEventListener("click",X=>{X.preventDefault(),M[L]?.scrollIntoView({behavior:"smooth",block:"start"})}))}),v.showFooter&&(g.footerLeft||g.footerRight||g.footerCenter)){let b=activeDocument.createElement("div");if(b.setCssStyles({position:"absolute",bottom:"0",left:"0",right:"0",height:`${c}px`,display:"flex",alignItems:"center",...v.showFooterBorder?{borderTop:`0.5px solid ${w}33`}:{},padding:`0 ${d}px 0 ${r}px`,fontSize:"9px",color:"#aaa",fontFamily:m}),g.footerCenter){let x=activeDocument.createElement("span");x.className="mpdf-hf-center",x.textContent=g.footerCenter,b.appendChild(x)}else{let x=activeDocument.createElement("span");x.textContent=g.footerLeft,b.appendChild(x);let L=activeDocument.createElement("span");L.className="mpdf-hf-right",L.textContent=g.footerRight,b.appendChild(L)}D.appendChild(b)}}}async exportPDF(){let n=this.plugin.settings;this.exportBtn.disabled=!0,this.exportBtn.textContent="\u2B07 Exporting\u2026";let e=()=>{this.exportBtn.disabled=!1,this.exportBtn.textContent="\u2B07 Export PDF"};if(!this.layoutCache){this.renderDebounceTimer!==null&&(window.clearTimeout(this.renderDebounceTimer),this.renderDebounceTimer=null);let u=++this.renderToken;this.showLoading();try{await this.doRender(u)}catch(g){let y=g instanceof Error?g.message:String(g);new l.Notice("Render error: "+y),this.hideLoading(),e();return}}let a=this.layoutCache;if(!a||a.layouts.length===0){new l.Notice("Nothing to export."),e();return}let{layouts:i,pw:s,ph:o,mTop:r,mLeft:d,mRight:c,footerH:p,headerH:h,contentW:f,docCSS:m,fontFamily:w,accentColor:T,pageBackground:S,isRTL:v}=a,k=i.map(u=>{let g=u.pageNodes.map(O=>{let b=O.cloneNode(!0);return b.querySelectorAll("style, script").forEach(x=>{x.closest("svg")||x.remove()}),b.outerHTML}).join(`
`),y=n.showHeader&&(u.headerLeft||u.headerCenter||u.headerRight),E=u.headerCenter?`<span style="flex:1;text-align:center;">${P(u.headerCenter)}</span>`:`<span>${P(u.headerLeft)}</span><span style="margin-left:auto;">${P(u.headerRight)}</span>`,C=y?`<div style="position:absolute;top:${r*.4}px;left:${d}px;right:${c}px;display:flex;align-items:center;font-size:9px;color:#999;font-family:${w};white-space:nowrap;">${E}</div>`:"",F=u.footerCenter?`<span style="flex:1;text-align:center;">${P(u.footerCenter)}</span>`:`<span>${P(u.footerLeft)}</span><span style="margin-left:auto;">${P(u.footerRight)}</span>`,B=n.showFooter&&(u.footerLeft||u.footerRight||u.footerCenter),D=n.showFooterBorder?`border-top:0.5px solid ${T}33;`:"",$=B?`<div style="position:absolute;bottom:0;left:0;right:0;height:${p}px;display:flex;align-items:center;${D}padding:0 ${c}px 0 ${d}px;font-size:9px;color:#aaa;font-family:${w};">${F}</div>`:"",H=`<div class="mpdf-doc"${v?' dir="rtl"':""} style="position:absolute;top:${r+h}px;left:${d}px;width:${f}px;">${g}</div>`;return`<div class="mpdf-export-page">${C}${H}${$}</div>`}),M=`
